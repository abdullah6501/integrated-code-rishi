...
function User() {
  ...
  const fetchData1 = async () => {
    try {
      const response = await fetch(
        `http://192.168.0.166:3000/project_infouser?email=${Email}`
      );
  ...
  const searchId = useCallback(async () => {
    try {
      const response = await fetch(
        `http://192.168.0.166:3000/project_info/${userId}`
      );
  ...
  const saveChanges = async () => {
    try {
      ...
      await axios.put(
        `http://192.168.0.166:3000/api/update/${dataToSend.Title}`,
        dataToSend
      );
      setEditMode(false);
      setSuccess(true);
  ...
  const handleTaskButtonClick = async (title) => {
    try {
      const response = await fetch(
        `http://192.168.0.166:3000/task_details/${title}`
      );
